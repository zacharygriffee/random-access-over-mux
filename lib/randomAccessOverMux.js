...
import {injectDependencies as injectDependenciesForStreamSetup, setupStream} from "./setupStream.js";
import {handleCallback} from "./handleCallback.js";

const symbol = Symbol.for["random access over mux"];
let b4a, ProtomuxRPC, cenc;
...
export async function injectDependencies(deps, customResolver) {
    await injectDependenciesForStreamSetup(deps);
    await injectDependenciesForMessages(deps);
    ({b4a, ProtomuxRPC, cenc} = await inject(deps, customResolver));
}
